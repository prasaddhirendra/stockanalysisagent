...
import pandas as pd
from src.market_analysis_crew import MarketAnalysisCrew, LLM_PROVIDERS, get_available_providers
...
def main():
    ...
    with st.sidebar:
        ...
        ticker = st.text_input("Enter Stock Ticker:", value="RELIANCE",).upper()
        ...
        time_period = st.selectbox(
            ...
            [ "1mo", "3mo", "6mo","1y","3y" ],
            index=0,
            help="Select how far back the analysis should go: 3 year (3y), 1 year (1y), 6 months (6mo), 3 months (3mo), or 1 month (1mo). This affects the historical data used in the analysis."
        )
        st.markdown("""
            <div class="sidebar-tooltip">
    ...
    if hasattr(st.session_state, 'analysis_results'):
        ...
        if isinstance(stock_data, str):
            try:
                ...
                st.error(f"Failed to parse stock data: {e}")
                stock_data = {}
        ...
        with cols[2]:
            st.metric("52-Week High", f"${stock_data.get('52_week_high',0):.2f}")
        with cols[3]:
            st.metric("52-Week Low", f"${stock_data.get('52_week_low',0):.2f}")
        st.markdown('</div>', unsafe_allow_html=True)
